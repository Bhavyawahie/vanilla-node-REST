...
const { getAllProducts, getProductById, addNewProduct, deleteProduct } = require('./controllers/productController')

const server = http.createServer((req, res) => {
    if(req.url === "/api/v1/products" && req.method === "GET"){
        getAllProducts(req, res)
    } else if (req.url.match(/\/api\/v1\/products\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$)||([0-9]+)([0-9]+)/) && req.method === "GET") {
